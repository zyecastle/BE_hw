...
def result(request):
    ...
    return render(request, 'result.html', {'alltext':entered_text, 'dictionary':word_dictionary.items(), 'total' : total_word_count,
    'total_chars': total_char_count, 'no_space': char_no_space, 'most_words': most_frequent_words, 'max_freq': max_freq,})

def hello(request):
    name = request.GET.get('name')
    return render(request, 'hello.html', {'username': name})